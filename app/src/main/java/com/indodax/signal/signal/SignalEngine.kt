...
object SignalEngine {
    // BUY/SELL require all four specified confirmations; HOLD reports partial confirmation.
    fun evaluate(s: Snapshot): SignalResult {
        ...
        val confidence = maxOf(buyChecks.count { it }, sellChecks.count { it }) * 25
        ...
        val vol = when {
            s.volumeAverage == 0.0 -> "volume unavailable"
            volume -> "volume above average"
            s.volume == s.volumeAverage -> "volume at average"
            else -> "volume below average"
        }
